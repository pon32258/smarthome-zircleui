...
export default {
  ...
  methods: {
    initMap() {
      this.map = L.map(this.$zircle.getCurrentViewName(), {
        attributionControl: false,
        zoomControl: false
      }).setView([38.63, -90.23], 14);
      L.tileLayer(
      ...
      this.markers.push(
        L.marker([38.6, -90.2])
          .bindPopup('Mary at work')
          .addTo(this.map),
        L.marker([38.63, -90.23])
          .bindPopup('John at home.')
          .addTo(this.map)
          .openPopup(),
        L.marker([38.66, -90.26])
          .bindPopup('Peter at school')
          .addTo(this.map)
      );
      this.active = 1;
      this.showMe(1, 'green');
